...
import { PAPER_PRESETS, DEFAULT_VERITAS_LOGO } from '../data/defaultData';
import {
  X,
  Building,
  Image as ImageIcon,
...
export const SchoolConfigModal: React.FC<SchoolConfigModalProps> = ({
  ...
  const [svgTextInput, setSvgTextInput] = React.useState('');
  const [showSvgInput, setShowSvgInput] = React.useState(false);
  ...
  const handleApplySvgText = () => {
    if (!svgTextInput.trim()) return;
    const cleanText = svgTextInput.trim();
    let dataUrl = cleanText;
    if (cleanText.includes('<svg')) {
      dataUrl = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(cleanText);
    }
    onUpdateSchoolConfig({
      ...schoolConfig,
      logoDataUrl: dataUrl,
                ...
                <div className="space-y-2 flex-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      className="inline-flex items-center space-x-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded font-semibold transition-colors shadow-xs"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center space-x-1.5 px-3 py-1.5 bg-white hover:bg-slate-100 text-slate-700 rounded border border-slate-200 font-semibold transition-colors"
                    >
                      <ImageIcon className="w-3.5 h-3.5 text-indigo-600" />
                      <span>{showSvgInput ? 'Ocultar Código SVG' : 'Pegar SVG'}</span>
                    </button>
                    ...
                    <button
                      ...
                      className="inline-flex items-center space-x-1 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded border border-slate-200 transition-colors"
                    >
                      <RotateCcw className="w-3.5 h-3.5" />
                      <span>Logo Veritas</span>
                    </button>
                  </div>

                  {/* SVG Code or URL Paste Box */}
                  {showSvgInput && (
                    <div className="p-3 bg-white rounded-lg border border-slate-200 space-y-2 mt-2">
                      <label className="block text-[11px] font-bold text-slate-700">
                        Pega el contenido XML de logo.svg o Data URL:
                      </label>
                      <textarea
                        rows={3}
                        placeholder='<svg xmlns="http://www.w3.org/2000/svg" ...> o data:image/svg+xml;base64,...'
                        value={svgTextInput}
                        onChange={(e) => setSvgTextInput(e.target.value)}
                        className="w-full bg-slate-50 text-slate-800 text-[11px] font-mono p-2 rounded border border-slate-200 focus:outline-none focus:border-indigo-500"
                      />
                      <div className="flex justify-end space-x-2">
                        ...
                        <button
                          type="button"
                          onClick={handleApplySvgText}
                          className="px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded shadow-2xs"
                        >
                          Aplicar Logo SVG
                        </button>
                      </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(Object.keys(PAPER_PRESETS) as PaperSizeKey[]).map((key) => {
                    ...
                    return (
                      ...
                      >
                        <div className="font-bold text-[11px] capitalize">{key}</div>
